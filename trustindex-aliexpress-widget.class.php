...
class TrustindexWidget_aliexpress extends WP_Widget {
...
{
parent::__construct(
'trustindex_aliexpress_widget',
'Trustindex - AliExpress reviews widget',
[
'classname' => 'trustindex-widget',
'description' => 'Embed AliExpress reviews fast and easily into your WordPress site. Increase SEO, trust and sales using AliExpress reviews.'
]
);
}
...
{
global $wpdb;
global $trustindex_pm_aliexpress;
if ($trustindex_pm_aliexpress->is_enabled()) {
extract($args);
echo $before_widget;
$wasError = false;
...
echo $trustindex_pm_aliexpress->renderWidgetFrontend($instance['ti-widget-ID']);
} else if ($trustindex_pm_aliexpress->is_noreg_linked()) {
echo $trustindex_pm_aliexpress->renderWidgetFrontend();
} else {
echo $trustindex_pm_aliexpress->frontEndErrorForAdmins(sprintf(__("Please fill out <strong>all the required fields</strong> in the <a href='%s'>widget settings</a> page", 'trustindex-plugin'), admin_url('admin.php?page='.$trustindex_pm_aliexpress->get_plugin_slug().'/settings.php')));
}
echo $after_widget;
} else {
}
...
global $wp_version;
global $trustindex_pm_aliexpress;
$tiWidgets = $trustindex_pm_aliexpress->get_trustindex_widgets();
...
<?php if ($trustindex_pm_aliexpress->is_trustindex_connected()): ?>
<?php if ($tiWidgets): ?>
...
<?php endforeach; ?>
<?php else: ?>
<?php echo TrustindexPlugin_aliexpress::get_alertbox('warning',
...
<img src="<?php echo $trustindex_pm_aliexpress->get_plugin_file_url('static/img/help-where-is-id.jpg'); ?>" alt="ID column here: <?php echo 'https://admin.trustindex.io/'; ?>widget" />
</div>
<?php else: ?>
<?php echo TrustindexPlugin_aliexpress::get_alertbox('warning',
...
. sprintf(__("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", 'trustindex-plugin'), admin_url('admin.php?page='.$trustindex_pm_aliexpress->get_plugin_slug().'/settings.php&tab=setup_trustindex'))
); ?>
<?php endif; ?>
</div>
<?php
